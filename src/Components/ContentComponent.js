...
export default class ContentComponent extends Component {
    ...
    SetGlassesModel(newGlasses) {
        this.setState({ glasses: newGlasses })
    }
    RenderGlassesList(glasses) {
        return (
            <div key={glasses.id} onClick={() => {
                console.log(glasses);
                this.SetGlassesModel(glasses)
                ...
                <div className='content container'>
                        ...
                        <div className="model__right col-6">
                            ...
                            <div className="glasses-card">
                                ...
                                <div className="glasses-info">
                                    <h5>{glasses.name}</h5>
                                    <button className="btn-danger p-1">{glasses.price}$</button> <span className="text-success p-1"> Stocking</span>
                                    <p>{glasses.desc} </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="glasses-list row">
                        {this.arrGlasses.map((item, index) => {
                            return this.RenderGlassesList(item)
                        })}
                    </div>
                </div>
            </div>
